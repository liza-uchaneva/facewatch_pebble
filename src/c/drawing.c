...
float s_sad_phase = 0.0f;
float s_smile_phase = 0.0f; // 0 = neutral, 1 = full smile
bool s_is_smile_animating = false;
bool s_is_sadness_animating = false;
...
static void draw_eye(GContext *ctx, int side) {
  ...
  if (s_is_blinking) {
    if(s_is_sadness_animating)
    {
      graphics_context_set_fill_color(ctx, COLOR_DARK_GRAY);
      graphics_fill_radial(ctx, GRect(base_x - 20, base_y - 20, 43, 43), 
      GOvalScaleModeFitCircle, 30, DEG_TO_TRIGANGLE(-90), DEG_TO_TRIGANGLE(90));
      return;
    }
    else{
      graphics_context_set_stroke_width(ctx, 6);
  ...
  if(s_sad_phase > 0.4 && s_is_sadness_animating)
  {
      graphics_context_set_fill_color(ctx, COLOR_DARK_GRAY);
      graphics_fill_rect(ctx, GRect(base_x - 15, base_y - 20, 30, 10 - 8 * s_sad_phase), 15, GCornersTop);
  }
}
...
static void draw_cheek(GContext *ctx, int side) {
  ...
  graphics_context_set_fill_color(ctx, COLOR_CORAL);
  graphics_fill_rect(ctx, GRect(point.x, point.y, 35, 25), 20, GCornersAll);
}
...
static void draw_brow(GContext *ctx, int side) {
  ...
  int base_y = screencenter.y - screencenter.y / 4 - 34;
...
static void draw_mouth(GContext *ctx) { 
  ...
  graphics_draw_arc(ctx, 
                    GRect(screencenter.x - 24, 
                          screencenter.y - 15 - (int)(2 * s_smile_phase), 50, 50),
                    GOvalScaleModeFitCircle, 
                    DEG_TO_TRIGANGLE(start_angle), 
                    DEG_TO_TRIGANGLE(end_angle));

  int top_start = -185 - 20 * s_sad_phase;
  int top_end = -170 - 25 * s_sad_phase;
  graphics_draw_arc(ctx, 
                    GRect(screencenter.x - 30, 
                          screencenter.y - 18 - (int)(2 * s_smile_phase), 65, 65),
                    GOvalScaleModeFitCircle, 
...
static void draw_lush(GContext *ctx, int side) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (s_smile_phase <= 0.5f) {
      // Draw neutral position
      x0 = base_x + neutral_offsets[i * 2][0] * side;
      y0 = base_y + neutral_offsets[i * 2][1];

      x1 = base_x + neutral_offsets[i * 2 + 1][0] * side;
      y1 = base_y + neutral_offsets[i * 2 + 1][1];
    } else {
      // Draw smile position
      x0 = base_x + smile_offsets[s][i * 2][0];
      y0 = base_y + smile_offsets[s][i * 2][1];

      x1 = base_x + smile_offsets[s][i * 2 + 1][0];
      y1 = base_y + smile_offsets[s][i * 2 + 1][1];
    }
...
void draw_dark_circles(GContext *ctx, int side){
  if(!s_is_sadness_animating) return;
...
void update_face_layer(int steps, int average_steps) {
  ...
  float ratio = (float)steps / (float)average_steps;

  // Clamp ratio to max 2.0 for sane upper bound
  if (ratio > 2.0f) ratio = 2.0f;

  // Reset both animation flags
  s_is_smile_animating = false;
  s_is_sadness_animating = false;

  if (ratio > 1.0f) {
    s_is_smile_animating = true;
    s_smile_phase = ratio - 1.0f; // range: 0.0 to 1.0
    s_sad_phase = 0.0f;
  } 
  else if (ratio < 1.0f) {
    s_is_sadness_animating = true;
    s_sad_phase = 1.0f - ratio; // range: 0.0 to 1.0
    s_smile_phase = 0.0f;
  } 
  else {
    // Neutral face
